...
public class WarmupStatusHandler implements LobbyStatusHandler {

    @Override
    public LobbyStatus getTargetStatus() {
        return LobbyStatus.WARMUP;
    }

    @Override
    public void handle(Lobby lobby) {
        log.info("Лобби с ID {} ожидает игроков (разминка)", lobby.getId());
        lobby.setWarmupAt(LocalDateTime.now());
    }
}
