...
public class PickedServerStatusHandler implements LobbyStatusHandler {

    @Override
    public LobbyStatus getTargetStatus() {
        return LobbyStatus.PICKING_SERVER;
    }

    @Override
    public void handle(Lobby lobby) {
        log.info("Лобби с ID {} начало выбор сервера", lobby.getId());
        lobby.setPickedServerAt(LocalDateTime.now());
    }
}
